 ...
 */
export function useAutoSave(jobId, delay = 3000) {
    ...
    const addPendingChange = (field) => {
        if (!pendingChanges.some(f => f.id === field.id)) {
            setPendingChanges(prev => [...prev, field]);
        }
        else {
            setPendingChanges(prev => prev.map(f => f.id === field.id ? field : f));
        }
    };
    // Save changes to backend
    const saveChanges = async () => {
        if (!jobId || pendingChanges.length === 0)
            return null;
        setIsSaving(true);
        setError(null);
        try {
